...
namespace TSVCEO.CloudPrint.Util
{
    ...
    {
        private static IEnumerable<dynamic> ReadJsonArray(JsonReader reader)
        {
            while (reader.Read())
            {
                ...
                {
                    yield return ReadJsonValue(reader, true);
                }
            }
        }
        ...
        private static dynamic ReadJsonValue(JsonReader reader, bool alreadyread = false)
        {
            while (alreadyread || reader.Read())
            {
                switch (reader.TokenType)
                {
                    case JsonToken.StartArray: return ReadJsonArray(reader).ToList();
        ...
        private static void WriteJsonObject(JsonWriter writer, IDictionary dict)
        {
